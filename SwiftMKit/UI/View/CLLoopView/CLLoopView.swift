...
public class CLLoopView: UIView,UIScrollViewDelegate {
    ...
    private func initializeUI(){
        ...
        loopScrollView.contentSize = CGSizeMake(width * 3.0, 0)
        ...
        loopScrollView.pagingEnabled = true
//        loopScrollView.decelerationRate = 0.85
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(CLLoopView.tapGestureHandle(_:)))
    ...
    func loadImage(fromUrl:String? ,toImagev : UIImageView){
        ...
        toImagev.hnk_setImageFromURL(NSURL(string: fromUrl ?? "")!, placeholder:UIImage(named:"view_default_loading"), format: Format<UIImage>(name: "original")) {
            [weak self] image in
    ...
    private func updateImageData(){
        ...
        if arrImageUrl.count > 0{
            if currentPage == 0{
                loadImage(arrImageUrl.last, toImagev: imageView0)
                loadImage(arrImageUrl[currentPage], toImagev: imageView1)
                loadImage(arrImageUrl[currentPage + 1], toImagev: imageView2)
            }else if currentPage == arrImageUrl.count - 1{
                loadImage(arrImageUrl[currentPage - 1], toImagev: imageView0)
                loadImage(arrImageUrl[currentPage], toImagev: imageView1)
                loadImage(arrImageUrl.first, toImagev: imageView2)
            }else{
                loadImage(arrImageUrl[currentPage - 1], toImagev: imageView0)
                loadImage(arrImageUrl[currentPage], toImagev: imageView1)
                loadImage(arrImageUrl[currentPage + 1], toImagev: imageView2)
    ...
    private func startTimer(){
        if autoShow{
            timer = nil
            timer = NSTimer.scheduledTimerWithTimeInterval(timeInterval, target: self, selector: #selector(CLLoopView.autoTurnNextView), userInfo: nil, repeats: true)
        }
    }
