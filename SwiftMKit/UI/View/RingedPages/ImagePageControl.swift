...
public class ImagePageControl: UIControl {
    ...
    public var indicatorTintColor = UIColor.lightGrayColor()     // will be ignored if pageIndicatorImage setted
    public var currentIndicatorTintColor = UIColor.blueColor()   // will be ignored if currentPageIndicatorImage setted
    ...
    public override func drawRect(rect: CGRect) {
        renderPages(UIGraphicsGetCurrentContext(), rect: rect)
    }
...
private extension ImagePageControl {
    func p_Initialize() {
        ...
        contentMode = .Redraw
        addTarget(self, action: #selector(valueDidChanged), forControlEvents: .ValueChanged)
    }
    ...
    func renderPages(context: CGContext?, rect: CGRect) {
        ...
        for i in 0..<numberOfPages {
            ...
            if let image = image {// draw the images
                ...
                image.drawAtPoint(CGPoint(x: centeredXOffset, y: yOffset))
                ...
                if i == displayedPage {
                    CGContextSetFillColorWithColor((context)!,currentIndicatorTintColor.CGColor)
                } else {
                    CGContextSetFillColorWithColor((context)!,indicatorTintColor.CGColor)
                }
    ...
    func sizeForNumber( number: Int) -> CGSize {
    ...
    func setCurrentPage( page: Int, sendEvent: Bool, canDefer: Bool) {
